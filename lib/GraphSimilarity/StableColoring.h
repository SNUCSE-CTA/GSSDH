...
namespace GraphLib {
    ...
    /**
     ...
     Efficient color refinement (also known as 1-WL test, Naive vertex classification) by Berkholz, Bonsma and Grohe [1]
     Time complexity: O((V + E) log V), Space complexity: O(V + E)
     @note Current implementation only works for undirected graphs, and does not produce canonical coloring
           (i.e., it is possible that the coloring for two isomorphic graphs differ up to permutation)
           This is to avoid sorting, thereby making the algorithm more efficient for pairwise color refinement (used for Graph Isomorphism [2]).

           Tested with tree isomorphism problem from Estonian Informatics Olympiad 2018, Iso testcases.

     References
       1. C. Berkholz, P. Bonsma, and M. Grohe.
            Tight Lower and Upper Bounds for the Complexity of Canonical Colour Refinement. Theory of Computing Systems 2017.
       2. G. Gu, Y. Nam, K. Park, Z. Galil, G. F. Italiano, and W.-S. Han,
            Scalable Graph Isomorphism: Combining Pairwise Color Refinement and Backtracking via Compressed Candidate Space, ICDE 2021
     */
    ...
    public:
        ...
        inline std::vector<int>& GetVerticesByColor(int c) {return color_partition[c];}
    ...
    public:
        PairwiseStableColoring(LabeledGraph *g1, LabeledGraph *g2, LabeledGraph *combined) : StableColoring(combined) {
        ...
        bool CheckColorCount() {
            return color_count_equal;
        }
        ...
        inline int GetNumVertexClass(int u) {
            return left_color_count[color[u]];
        }
        ...
        void PrintEntireColorPartition(int upto = 5) {
            ...
            for (int i = 0; i < std::min(GetNumColors(), upto); i++) {
                fprintf(stdout, "  Color %d: %d left, %d right\n", i, GetLeftColorCount(i), GetRightColorCount(i));
