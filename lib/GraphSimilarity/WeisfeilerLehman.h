...
//            }
    ...
    WeisfeilerLehman(LabeledGraphDatabaseEntry *g) {
        G = g;
    ...
    void Match(std::vector<int>& mapping) {
        ...
        while (!color_queue.empty()) {
            ...
            if (DEBUG)
                printf("Check color %d (L, R = %d, %d)\n", c, num_left[c], num_right[c]);
            color_queue.pop();
            if (num_left[c] > 0 and num_right[c] > 0) {
//                std::sort(color_partition[c].begin(), color_partition[c].end(), [&](int a, int b) -> bool {
//                    if (a < VL and b < VL)
...
//                    }
            ...
            for (auto x : color_partition[c]) {
                ...
                if (DEBUG)
                    printf("move vertex %d to color %d\n", x, p);
            }
