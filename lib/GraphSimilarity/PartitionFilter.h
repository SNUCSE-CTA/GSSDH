...
struct MismatchingStructure {
  ...
  MismatchingStructure(std::vector<int> vertices_, std::vector<int> edges_)
...
class PartitionFilter {
 ...
 public:
  PartitionFilter(GSSEntry *query_ = nullptr, GSSEntry *data_ = nullptr)
      : query(query_), data(data_) {
    BuildPartition();
  };

  int GetPartitionBound() { return mismatches.size(); }

  void BuildPartition() {
    //            fprintf(stderr, "Building partition for %d
    ...
    for (int i = 0; i < data->GetNumVertices(); i++) {
      for (int e : data->GetIncidentEdges(i)) {
        size_2_label_freq.emplace(data->GetVertexLabel(i),
                                  data->GetEdgeLabel(e));
      }
    }
    for (int i = 0; i < query->GetNumVertices(); i++) {
      ...
      for (int e : query->GetIncidentEdges(i)) {
        ...
        if (size_2_label_freq.find(
                {query->GetVertexLabel(i), query->GetEdgeLabel(e)}) ==
            size_2_label_freq.end()) {
      ...
      for (int i = 0; i < inc.size(); i++) {
        ...
        for (int j = i + 1; j < inc.size(); j++) {
          ...
          if (query->GetEdgeLabel(inc[i]) == query->GetEdgeLabel(inc[j])) {
            ...
            if (!found) {
              mismatches.emplace_back(std::vector<int>{v},
                                      std::vector<int>{inc[i], inc[j]});
              used_vertex[v] = 1;
