            ...
            while (!stk.empty()) {
                ...
                if (current.u == -1) {
                    ...
                    for (int x : coloring->GetVerticesByColor(u_color)) {
                        if (x >= VL)
                            current.v_candidates.push_back(x);
                    }
                }
                while (current.v_candidate_index < current.v_candidates.size()) {
                    ...
                    if (ok) {
                        ...
                        while (next_depth < matching_order.size() and coloring->GetNumVertexClass(matching_order[next_depth]) == 1) {
                            next_depth++;
                        }
