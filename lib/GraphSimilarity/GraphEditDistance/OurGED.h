...
namespace GraphLib::GraphSimilarity {
    ...
    struct OurState : public State {
        ...
        OurState(const OurState &other): State(other) {
            ...
            std::memcpy(inverse_mapping, other.inverse_mapping, sizeof(int) * NumG2Vertices);
        }
        ...
        std::string to_string() const {
            ...
            oss << "| Depth = " << depth << " | Cost = " << cost << " | Lb = " << lower_bound << " | Similarity = " << similarity;
            return oss.str();
        }
    };
    ...
    class OurGED: public GraphEditDistanceSolver {
        std::priority_queue<OurState*, std::vector<OurState*>, OurStateComparator> queue;
...
//                fprintf(stderr, "  Consider %d-%d: cost %d + future lb %d = %d\n", u, v, child_cost, lb, child_cost+lb);
...
//                fprintf(stderr,"  Push state %d with bound %d\n", child_state->id, child_state->lower_bound);
        ...
        int GED() {
            ...
            branch_similarities.resize(G1->GetNumVertices(), std::vector<int>(G2->GetNumVertices(), 0));
            for (int i = 0; i < G1->GetNumVertices(); i++) {
                for (int j = 0; j < G2->GetNumVertices(); j++) {
                    branch_similarities[i][j] = BranchSimilarity(G1->GetBranch(i), G2->GetBranch(j));
                }
            }
            ...
            while (!queue.empty()) {
                ...
                if (threshold >= 0) {
                    if (current_best < threshold) {
                        queue = std::priority_queue<OurState*, std::vector<OurState*>, OurStateComparator>();
                        break;
                    }
